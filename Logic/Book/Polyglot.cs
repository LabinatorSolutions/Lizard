...
namespace LTChess.Logic.Book
{
    ...
    public unsafe class Polyglot
    {
        ...
        public static PolyglotSelectionMethod DefaultMethod = PolyglotSelectionMethod.Random;

        static Polyglot()
        {

#if DEBUG
            //Log("Read " + BookEntries.Count + " book entries");
#endif
        }

        public static bool Probe(in Position pos, PolyglotSelectionMethod method, out Move move, out int weight)
        {
            move = Move.Null;
            weight = 0;
            try
            {
                if (InternalProbeDisk(pos, out List<PolyglotEntry> list))
                {
                    PolyglotEntry entry;
                    if (method == PolyglotSelectionMethod.Best)
                    {
                        entry = list.OrderByDescending(x => x.Weight).First();
                    }
                    else
                    {
                        entry = list[rand.Next(list.Count)];
                    }

                    weight = entry.Weight;

                    Span<ScoredMove> legal = stackalloc ScoredMove[MoveListSize];
                    ...
                    for (int i = 0; i < size; i++)
                    {
                        ...
                        if (m.To == entry.ToSquare && m.From == entry.FromSquare)
                        {
                            if (entry.PromotionTo == 0 || m.PromotionTo == entry.PromotionTo)
                            {
                                move = m;
                                return true;
                            }
                            ...
                            {
                                Log("WARN Discarding possible match " + m.ToString(pos) + ", entry.PromotionTo == " + PieceToString(entry.PromotionTo + 1));
                            }
                        }
                    }
                }
        ...
        private static bool InternalProbe(in Position pos, out List<PolyglotEntry> entries)
        {
            ulong posKey = MakeKey(pos);
            return BookEntries.TryGetValue(posKey, out entries);
        }

        /// <summary>
        /// Computes the hash of the Position <paramref name="pos"/>, and returns true if 
        /// that position has 1 or more Polyglot entries, setting the list <paramref name="entries"/> accordingly.
        /// <br></br>
        /// Each time this method is called, the entirety of the Polyglot file will be read through.
        /// This can save some ram usage, but can really start to slow things down if the file is too large
        /// </summary>
        private static bool InternalProbeDisk(in Position pos, out List<PolyglotEntry> entries)
        {
            ...
            {
                int streamPos = 0;
                while (streamPos < bookData.Length)
                {
                    byte[] buff = bookData[streamPos..(streamPos + PolyglotEntry.EntrySize)];
                    streamPos += PolyglotEntry.EntrySize;

                    PolyglotEntry entry = new PolyglotEntry();
                    entry.Key = BitConverter.ToUInt64(buff, 0);

                    if (BitConverter.IsLittleEndian)
                    {
                        entry.Key = BinaryPrimitives.ReverseEndianness(entry.Key);
                    }
                    ...
                    entry.RawMove = BitConverter.ToUInt16(buff, 8);
                    entry.Weight = BitConverter.ToUInt16(buff, 10);
                    entry.Learn = BitConverter.ToUInt32(buff, 12);

                    //  Polyglot files are supposed to be Big Endian, so if our machine is Little Endian
                    //  then BitConverter read the binary data incorrectly and we have to reverse it.
                    if (BitConverter.IsLittleEndian)
                    {
                        entry.RawMove = BinaryPrimitives.ReverseEndianness(entry.RawMove);
                        entry.Weight = BinaryPrimitives.ReverseEndianness(entry.Weight);
                        entry.Learn = BinaryPrimitives.ReverseEndianness(entry.Learn);
                    }
            ...
            Log("Disk in " + sw.Elapsed.TotalMilliseconds);
            return entries.Count != 0;
        }

        private static ulong MakeKey(in Position pos)
        {
            Bitboard bb = pos.bb;
            ...
            {
                if (bb.PieceTypes[idx] == Piece.None)
                {
                    continue;
                }
            ...
            ulong turnHash = (pos.ToMove == Color.White) ? RandomTurn[0] : 0;

            ulong key = pieceHash ^ castleHash ^ enPassantHash ^ turnHash;

            Log("FEN = " + pos.GetFEN());
            Log("Key = " + key.ToString("X"));
            return key;
        }

        private static int PieceToPolyglotPiece(int pc, int pt)
        {
            if (pc == Color.White)
            {
                return (pt * 2) + 1;
        ...
        private static readonly ulong[] RandomPiece =
        {
        ...
        private static readonly ulong[] RandomCastle =
        {
            0x31D71DCE64B2C310, 0xF165B587DF898190, 0xA57E6339DD2CF3A0, 0x1EF6E6DBB1961EC9,
        };

        private static readonly ulong[] RandomEnPassant =
        {
            0x70CC73D90BC26E24, 0xE21A6B35DF0C3AD7, 0x003A93D8B2806962, 0x1C99DED33CB890A1,
            0xCF3145DE0ADD4289, 0xD0E4427A5514FB72, 0x77C621CC9FB3A483, 0x67A34DAC4356550B,
        };

        private static readonly ulong[] RandomTurn =
        {
            0xF8D626AAAF278509,
        };
    ...
    public enum PolyglotSelectionMethod
    {
        Random,
        Best
    }
